...
return;
    ...
    const handleZoomEnd = (event: d3.D3ZoomEvent<SVGSVGElement, unknown>) => {
      ...
      const newStart = Math.floor(visibleDomain[0]);
      const newEnd = Math.ceil(visibleDomain[1]);

      console.log('Zoom end - updating view bounds:', { newStart, newEnd, visibleDomain });
      setCurrentViewStart(newStart);
      setCurrentViewEnd(newEnd);
    };
  ...
  useEffect(() => {
    if (chartDataHook.chartData.length > 0 && currentViewEnd > 0) {
      ...
      const isValidView =
        ...
        currentViewEnd < chartDataHook.chartData.length;

      // Create chart if it doesn't exist yet, or if view has changed significantly and is valid
  ...
  // Update chart when view changes (panning) - disabled to prevent empty chart bug
  // The chart will show the data based on current view indices when it's created
  // useEffect(() => {
  //   if (chartDataHook.chartData.length > 0 && currentViewEnd > 0 && chartExistsRef.current) {
  //     // Only update if not currently loading data and not creating chart
  //     if (!isDataLoadingRef.current && !isLoadingMoreData && !isCreatingChartRef.current) {
  //       const now = Date.now();
  //       const timeSinceLastCreation = now - lastChartCreationRef.current;

  //       // Only update if enough time has passed since last creation
  //       if (timeSinceLastCreation >= CHART_CREATION_DEBOUNCE) {
  //         console.log('Updating chart for view change - panning detected');
  //         lastChartCreationRef.current = now;
  //         createChart();
  //       }
  //     }
  ...
  //   return undefined; // Explicit return for linter
  // }, [currentViewStart, currentViewEnd, createChart, isDataLoadingRef, isLoadingMoreData]);
