...
const D3StockChart: React.FC<D3StockChartProps> = ({ symbol, onSymbolChange }) => {
  ...
  // Refs for tracking loading attempts and timing
  ...
  useEffect(() => {
    if (state.timeframe !== null) {
      isInitialLoadRef.current = true; // Reset for new symbol/timeframe
      actions.setData([]); // Clear existing data
      actions.setError(null);
      actions.setIsLoading(true);
      actions.setHasUserPanned(false); // Reset user panning state
  ...
  // Check if we need to load more data when panning (predictive loading)
  const checkAndLoadMoreData = useCallback(async () => {
    ...
    const bufferSize = 300; // Load more data when within 300 points of edge

    let shouldLoadLeft = false;
    let shouldLoadRight = false;

    // Check if we need more historical data (panning left)
    if (state.currentViewStart <= bufferSize && !chartDataHook.isLeftLoading && totalDataLength > 0) {
    ...
    if (
      state.currentViewEnd >= totalDataLength - bufferSize &&
    ...
    // If no loading is needed, return
    ...
    try {
      if (shouldLoadLeft) {
        ...
        actions.setIsLoadingMoreData(true);
        await chartDataHook.loadMoreDataLeft(symbol, state.timeframe);
      }

      if (shouldLoadRight) {
        lastLoadAttemptRef.current = { type: 'right', dataLength: totalDataLength };
        actions.setIsLoadingMoreData(true);
        await chartDataHook.loadMoreDataRight(symbol, state.timeframe);
      }
    } catch (error) {
      console.error('Failed to load data:', error);
    } finally {
      actions.setIsLoadingMoreData(false);
    }
  }, [
    symbol,
    state.timeframe,
    state.currentViewStart,
    state.currentViewEnd,
    chartDataHook,
    state.isLoadingMoreData,
    state.hasUserPanned,
    state.isLoading,
  ...
  useEffect(() => {
    if (
      ...
      // Only check for data loading if enough time has passed
      if (timeSinceLastCheck >= DATA_LOAD_CHECK_INTERVAL) {
  ...
  useEffect(() => {
    if (state.sortedData.length > 0) {
      const atRightEdge = state.currentViewEnd >= state.sortedData.length - 5; // Within 5 points of the end
      const now = Date.now();
      ...
      if (timeSinceLastCheck >= RIGHT_EDGE_CHECK_INTERVAL) {
        ...
        actions.setIsAtRightEdge(atRightEdge);
  ...
  }, [state.currentViewEnd, state.sortedData.length, state.isLive, actions]);
  ...
  const createChart = useCallback(() => {
    ...
    const handleZoomStart = () => {
      ...
      actions.setHasUserPanned(true); // Mark that user has started panning
    };
      ...
      <div className="p-4 border-t border-border">
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          <div className="flex items-center space-x-4">
            ...
            <span>
              View: {state.currentViewStart}-{state.currentViewEnd}
            </span>
          ...
          <div className="flex items-center space-x-2">
            ...
            {state.isAtRightEdge && !state.isLive && (
