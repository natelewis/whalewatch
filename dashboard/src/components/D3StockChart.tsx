...
// ============================================================================
    ...
    const D3StockChart: React.FC<D3StockChartProps> = ({ symbol, onSymbolChange }) => {
      ...
      const lastLoadAttemptRef = useRef<{ type: 'left' | 'right'; dataLength: number } | null>(
        null
      );
      ...
      const createChart = useCallback(() => {
        ...
        const handleZoom = (event: d3.D3ZoomEvent<SVGSVGElement, unknown>) => {
          ...
          const { newViewStart, newViewEnd } = updateCurrentView({
            transform,
            sortedData,
            setCurrentViewStart,
            setCurrentViewEnd,
        ...
        const handleZoomEnd = (event: d3.D3ZoomEvent<SVGSVGElement, unknown>) => {
          ...
          //   0,
          //   sortedData.length - CHART_DATA_POINTS - clampedPanOffset
          // );
          // const newViewEnd = Math.min(sortedData.length - 1, newViewStart + CHART_DATA_POINTS - 1);
      ...
      const getVisibleData = useCallback(
        (startIndex: number, endIndex: number) => {
          ...
          const actualEndIndex = Math.max(
            actualStartIndex,
            Math.min(endIndex, sortedData.length - 1)
          );
      ...
      useEffect(() => {
        if (chartDataHook.chartData.length > 0 && currentViewEnd > 0) {
          ...
          if (currentViewStart > currentViewEnd || currentViewEnd < 0) {
            console.warn(
              'Invalid view range in chart creation effect, resetting to valid values:',
              {
                currentViewStart,
                currentViewEnd,
                dataLength: chartDataHook.chartData.length,
