      ...
      switch (event.key) {
        ...
        case 'End':
          ...
          if (svgRef.current) {
            const svg = d3.select(svgRef.current);
            const dataLength = chartDataHook.chartData.length;
            const endTransform = d3.zoomIdentity.translate(-(dataLength - CHART_DATA_POINTS) * 4, 0);
            svg.transition().duration(500).call(
              d3.zoom<SVGSVGElement, unknown>().transform,
              endTransform
            );
  ...
  const createChart = useCallback(() => {
    ...
    const zoom = d3
      ...
      .on('zoom', (event) => {
        ...
        const newDataPanOffset = Math.max(0, Math.floor(transform.x / 4));
        
        // Only log when offset changes to reduce spam
        if (newDataPanOffset !== dataPanOffset) {
          console.log('Pan calculation:', {
        ...
        // Only update if the offset actually changed and throttle updates
        const now = Date.now();
        if (newDataPanOffset !== dataPanOffset && now - lastPanUpdateRef.current > 8) { // ~120fps for smoother updates
          console.log('Updating dataPanOffset:', { old: dataPanOffset, new: newDataPanOffset, transformX: transform.x });
          setDataPanOffset(newDataPanOffset);
          lastPanUpdateRef.current = now;
        }
        
        // Update view bounds for predictive loading
        const viewStartIndex = Math.max(0, Math.floor(transform.x / 4));
        const viewEndIndex = Math.min(totalDataLength - 1, Math.ceil(transform.x / 4 + visibleWidth / 4));
        ...
        g.select('.grid-x').call(d3.axisBottom(newXScale).tickSize(-innerHeight).tickFormat(() => '') as any);
        g.select('.grid-y').call(d3.axisRight(newYScale).tickSize(-innerWidth).tickFormat(() => '') as any);
        ...
        if (!crosshairX.empty() && !crosshairY.empty() && crosshairX.style('opacity') !== '0') {
          ...
          if (d) {
            crosshairX
              .attr('x1', newXScale(clampedIndex))
              .attr('x2', newXScale(clampedIndex));
            crosshairY
              .attr('y1', newYScale(d.close))
              .attr('y2', newYScale(d.close));
          }
        }
      })
      .on('end', () => {
    ...
    g.append('g')
      .attr('class', 'grid-y')
      .attr('transform', `translate(${innerWidth},0)`)
      .call(d3.axisRight(yScale).tickSize(-innerWidth).tickFormat(() => ''))
      .style('opacity', 0.3);
                ...
                <div className="flex justify-between items-center w-full">
                  ...
                  <div className="flex gap-3 text-sm">
                    <span className="text-muted-foreground">
                      O: <span className="font-mono text-foreground">{hoverData.data.open.toFixed(2)}</span>
                    </span>
                    <span className="text-muted-foreground">
                      H: <span className="font-mono text-foreground">{hoverData.data.high.toFixed(2)}</span>
                    </span>
                    <span className="text-muted-foreground">
                      L: <span className="font-mono text-foreground">{hoverData.data.low.toFixed(2)}</span>
                    </span>
                    <span className="text-muted-foreground">
                      C: <span className="font-mono text-foreground">{hoverData.data.close.toFixed(2)}</span>
                    </span>
                  </div>
                </div>
              ) : (
            ...
            {hoverData && (
              ...
              >
                <div className="text-sm">
                  <div className="font-semibold">{new Date(hoverData.data!.time).toLocaleString()}</div>
                  <div className="text-muted-foreground">
                    {hoverData.data!.open.toFixed(2)} / {hoverData.data!.high.toFixed(2)} / {hoverData.data!.low.toFixed(2)} / {hoverData.data!.close.toFixed(2)}
                  </div>
                </div>
              </div>
            )}
      ...
      <div className="p-4 border-t border-border">
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          <div className="flex items-center space-x-4">
            ...
            <span>View: {currentViewStart}-{currentViewEnd}</span>
