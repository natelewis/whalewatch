...
return prev;
      ...
      const visibleData = sortedData.slice(prev.viewStart, prev.viewEnd + 1);
      
      return { ...prev, visibleData };
    });
  }, []);
...
return;
  ...
  }, [symbol, timeframe, chartDataHook, updateChartState]);
...
return;
  ...
  }, [symbol, timeframe, chartState, chartDataHook, updateChartState]);
  ...
  useEffect(() => {
    if (chartDataHook.chartData.length > 0) {
      updateChartState({ rawData: chartDataHook.chartData });
    }
  }, [chartDataHook.chartData, updateChartState]);

  // Update visible data when raw data or view bounds change
  useEffect(() => {
    updateVisibleData();
  }, [chartState.rawData, chartState.viewStart, chartState.viewEnd, updateVisibleData]);
  ...
  // Load more data when needed
  useEffect(() => {
    if (chartState.rawData.length > 0 && chartState.hasMoreData) {
      loadMoreData();
    }
  }, [chartState.rawData.length, chartState.viewStart, chartState.viewEnd, loadMoreData]);
      ...
      <div className="p-4 border-t border-border">
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <div className="flex items-center space-x-4">
            ...
            <span>View: {chartState.viewStart}-{chartState.viewEnd}</span>
          </div>
          <div className="flex items-center space-x-4">
            <span>Zoom: {chartState.zoomLevel.toFixed(2)}x</span>
            <span>Pan: {chartState.panOffset.x.toFixed(0)}, {chartState.panOffset.y.toFixed(0)}</span>
