// Extended test file for QuestDBConnection with comprehensive coverage using real database
...
describe('QuestDBConnection - Extended Coverage', () => {
  ...
  describe('executeSchema', () => {
    it('should execute schema successfully', async () => {
      // This test verifies that the executeSchema method works with the real schema file
    ...
    it('should handle schema execution with existing tables', async () => {
      // First execution
      await connection.executeSchema();

      // Second execution should not fail (IF NOT EXISTS)
      await connection.executeSchema();

      // Verify tables still exist
      const result = await connection.query('SELECT * FROM stock_aggregates LIMIT 1');
      expect(result).toBeDefined();
    });
  });

  describe('resetAllData', () => {
    it('should reset all data successfully with no existing data', async () => {
    ...
    it('should handle tables with partitions', async () => {
      // First, create tables and insert data to create partitions
      await connection.executeSchema();

      // Insert data to create partitions
      await connection.query(`
        INSERT INTO stock_aggregates VALUES 
        ('AAPL', '2024-01-01T10:00:00Z', 100, 105, 95, 102, 1000, 101, 50)
      `);

      await connection.query(`
        INSERT INTO option_trades VALUES 
        ('AAPL240101C00100000', 'AAPL', '2024-01-01T10:00:00Z', 2.50, 100, '[]', 1, 1, 12345)
      `);
    ...
    it('should handle resetAllData with option_contracts table', async () => {
      ...
      // Verify tables exist after reset
      const result = await connection.query('SELECT * FROM option_contracts LIMIT 1');
      expect(result).toBeDefined();
    });
  });
